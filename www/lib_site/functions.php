...
function out_contact_form ($listing_id = 0, $is_property = false) {
    global $_SITE; ?>
    <form action="<?=$_SITE['section_paths']['contact']['path']?><?=isset($_GET['source'])?'?source='.$_GET['source']:''?>" method="post" class="form-validate" novalidate>
    <?	if ($listing_id) { ?>
    ...
    <?	} ?>
        ...
        <?  if (!$is_property) { ?>
            <div class="form-field-short">
                <input type="phone" name="phone" value="" placeholder="Контактный телефон" class="form-input-text required-either">
            </div>
            <div class="form-field-or">
                или
            </div>
            <div class="form-field-short">
                <input type="text" name="email" value="" placeholder="Email" class="form-input-text required-either">
            </div>
        <?  } else { ?>
            <input type="phone" name="phone" value="" placeholder="Контактный телефон *" class="form-input-text required">
            <input type="hidden" name="from_property" value="1">
        <?  } ?>
        </div>
        <textarea name="message" placeholder="Ваше сообщение<?=!$is_property?' *':''?>" class="form-input-text<?=!$is_property?' required':''?>"></textarea>
        <div class="form-field-line">
            <label><input type="checkbox" name="consent" class="required" /> Даю согласие на обработку <a href="/privacy/" class="g-under" target="_blank">персональных данных</a></label>
        </div>
        <button type="submit" value="1" class="form-input-submit g-button"><?=!$is_property?'Отправить сообщение':'Заказать звонок'?></button>
    </form>
<?
}

function captcha_validate($token) {
    if ($_SERVER['REMOTE_ADDR'] == '127.0.0.1') {
        return true;
    }

    if (empty($token)) {
        echo '{"code":403,"message":"Captcha error"}';
        return false;
    }

    $secretKey = "6Ld2xacUAAAAAOB_ZN-dAt_jJrZs8Y9AmgnY9_et";
